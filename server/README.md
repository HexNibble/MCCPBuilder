...
脚本安装 Python 3、独立系统用户、systemd 服务、Nginx 和可选的 Let's Encrypt
证书；服务仍只监听 `127.0.0.1:18080`。脚本不会开放防火墙、关闭 TLS 验证、
覆盖已有发布密钥或把密钥上传到任何位置。只安装本机服务时使用：
