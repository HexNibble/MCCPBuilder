...
{
    [Fact]
    public void LauncherConfig_HidesPurchaseLinkWhenMicrosoftLoginIsNotAllowed()
    {
        var project = new ProjectConfig
        {
            LoginProviders = [new() { Type = LoginProviderType.Offline, DisplayName = "离线登录" }]
        };

        using var document = JsonDocument.Parse(new LauncherConfigGenerator().Generate(project));

        Assert.False(document.RootElement.GetProperty("login")
            .GetProperty("showMinecraftPurchase").GetBoolean());
    }

    [Fact]
    public void LauncherConfig_ShowsPurchaseLinkOnlyWhenMicrosoftLoginIsAllowed()
    {
        var project = new ProjectConfig
        {
            LoginProviders = [new() { Type = LoginProviderType.Microsoft, DisplayName = "Microsoft 正版登录" }]
        };

        using var document = JsonDocument.Parse(new LauncherConfigGenerator().Generate(project));

        Assert.True(document.RootElement.GetProperty("login")
            .GetProperty("showMinecraftPurchase").GetBoolean());
    }
