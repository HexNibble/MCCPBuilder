...
public sealed class ForgeBrandingService
{
    ...
    public async Task ApplyToJarAsync(
        string jarPath,
        string displayText,
        CancellationToken cancellationToken = default)
    {
        if (!InputValidator.IsValidForgeMcpBrandingText(displayText))
        {
            throw new InvalidDataException(
                "自定义 Forge 标识不能为空、不能包含控制字符，且最多 48 个字符。");
        }
        if (!File.Exists(jarPath))
        {
            throw new FileNotFoundException("缺少 Forge Universal JAR。", jarPath);
        }
        await RewriteForgeJarAsync(jarPath, displayText.Trim(), cancellationToken);
    }
