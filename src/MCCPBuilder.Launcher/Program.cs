...
internal static class Program
{
    ...
    private static int Main(string[] args)
    {
        ...
        try
        {
            ...
            if ((config.OfficialGame.Enabled ||
                 !config.Resources.Delivery.Equals("CustomServer", StringComparison.OrdinalIgnoreCase)) &&
                !args.Contains("--run-generated", StringComparer.OrdinalIgnoreCase))
            {
        ...
        string javaExecutable)
    {
        ...
        {
            throw new InvalidOperationException(
                "无法从 Mojang/Forge 官方服务器安装游戏文件：" +
                window.Failure.Message,
                window.Failure);
        }
        if (!window.Succeeded)
        {
            throw new InvalidOperationException("官方游戏文件安装未完成。");
        }
    }
...
internal sealed class LauncherRuntimeConfig
{
    ...
    public OfficialGameRuntimeConfig OfficialGame { get; set; } = new();
    public ResourceRuntimeConfig Resources { get; set; } = new();
    public LaunchRuntimeConfig Launch { get; set; } = new();
    public LoginRuntimeConfig Login { get; set; } = new();
}

internal sealed class ResourceRuntimeConfig
{
    public string Delivery { get; set; } = "CustomServer";
    public string Manifest { get; set; } = @"LauncherConfig\Resources\downloads.json";
}

internal sealed class OfficialGameRuntimeConfig
{
    public bool Enabled { get; set; }
    public string MinecraftVersion { get; set; } = "";
    public string Manifest { get; set; } = @"LauncherConfig\Game\version.json";
    public string MinecraftRoot { get; set; } = ".minecraft";
    public string VersionDirectory { get; set; } = "";
    public string ClientJar { get; set; } = "";
    public int DownloadConcurrency { get; set; } = 16;
    public ForgeBrandingRuntimeConfig ForgeBranding { get; set; } = new();
}

internal sealed class ForgeBrandingRuntimeConfig
{
    public bool Enabled { get; set; }
...
{
    public bool ShowMinecraftPurchase { get; set; }
