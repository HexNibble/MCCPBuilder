...
构建器直接读取所选版本隔离目录中的 Minecraft/Forge JSON，从中解析 `mainClass`、Windows x64规则、libraries、JVM参数和游戏参数，再自动生成短小的 `LauncherConfig\launch.bat` 与 UTF-8 `LauncherConfig\launch.arguments.json`。版本 JSON 的运行元数据会保存为 `LauncherConfig\Game\version.json`，但 Minecraft/Forge JAR、assets、libraries 和 natives 不会被上传。Launcher 首次运行时只接受 Mojang 与 Forge 官方域名的 HTTPS 下载地址，并验证清单中的长度与 SHA-1。整个过程不要求选择 BAT，也不保存打包电脑的绝对路径。正常点击 `Launcher.exe` 时会直接读取参数配置并启动 Java，不再经过 BAT 或 `cmd.exe`；BAT只作为兼容和诊断入口保留。Launcher通过 `ProcessStartInfo.ArgumentList` 逐项传参，从而避开命令行长度限制、中文乱码及不同电脑安装路径差异。

“客户端内容”页的来源选项互斥：选择 Modrinth 时导入标准 `.mrpack`，启动器只接受 Modrinth 官方 CDN；选择 CurseForge 时导入标准整合包 ZIP，打包器使用仅保存在当前进程内存中的 CurseForge API Key 调用官方 API 解析下载地址，Key 不写入项目、日志、Payload 或 Launcher；选择“上传到指定更新服务器”时，模组、资源包和光影包随当前 MCCP 更新服务器发布。Modrinth/CurseForge 整合包的 overrides 会作为配置内容加入服务器包。当前不提供 Minecraft/Forge 第三方镜像。
...
Forge 客户端可启用“自定义整行 Forge 标识”，把标题界面的 `MCP 版本号` 整行替换为自定义文字，不再保留 `MCP` 前缀。官方文件模式会先从 Forge 官方地址下载 Universal JAR，再修改最终用户本机副本中的清单及 `BrandingControl` 类常量；不会修改源客户端，也不会向服务器上传 Forge JAR。它可能不兼容会校验 Forge JAR 哈希的反作弊，因此默认关闭。
...
当 `versions` 中只有一个有效版本时会自动选中；有多个版本时必须再选择具体版本隔离目录。扫描范围仍以 `.minecraft` 为根，但官方模式会从发布包中剔除 `libraries`、`assets`、版本 JAR/JSON 与 natives；`versions` 下也只处理已选中的具体版本，其他兄弟版本目录会自动排除。
...
- `output\ClientPayload\.minecraft\versions\...`（只含允许发布的配置/模组内容，不含官方游戏 JAR）
...
登录区域依次允许勾选 Microsoft 正版、离线、标准 Authlib Injector 和统一通行证。最终 Launcher启动时必须由用户选择登录方式：离线登录必须主动选择并填写游戏名；标准 Authlib和统一通行证通过 HTTPS Yggdrasil即时认证。统一通行证会先 GET服务器ID配置并采用返回的 `apiRoot`，随后调用 `authserver/authenticate`，启动时同时添加 `-javaagent:nide8auth.jar=服务器ID` 与 `-Dnide8auth.client=true`，并提供官方注册页面入口。只有打包者允许 Microsoft 登录且最终用户当前选中 Microsoft 登录时，登录窗口才显示 Minecraft 官方购买入口；离线、Authlib 与统一通行证模式不显示，也不存在按启动次数弹出的购买提示。
...
项目与 Microsoft、Mojang、Minecraft Forge、Modrinth、CurseForge、HMCL、PCL2
及各认证服务不存在隶属或背书关系。本仓库和官方 Release 不分发 Minecraft 本体、
第三方模组、账号凭据或发布密钥；打包者仍须自行确认游戏 EULA、服务条款和每项内容
的再分发许可。完整项目边界、无担保、登录兼容及服务器运维责任见
[DISCLAIMER.md](DISCLAIMER.md)。该声明用于降低误解，不能替代针对具体业务的法律意见。
